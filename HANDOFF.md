...
- `src/shading.ts` installs one `onBeforeCompile` hook on `MeshStandardMaterial` for every material in the scene: palette lookup, cascaded shadow defines, height-weighted fog, cloth and foliage sway, GPU tree growth, staggered window lighting, flicker, wet surfaces, drifting cloud shadows, and weathering. Shared uniforms live in `presentationUniforms`.
- Cloud shadows are a two-octave value-noise field over world XZ that dims every directional light (sun and moon) and the water's caustics and glitter. The wind pushes the field; cover rises and falls over minutes and follows the overcast state.
- Cloth reads its anchor from the `aRetractTop` attribute: washing swings at the hem, flags stay fixed along the pole and ripple toward the fly end. The breeze is slow, and the ripple slope tilts the vertex normal so the wave shows in the shading.
- Weathering reads the `aBuildingAge` attribute (founding hour plus one, 0 for anything that does not age) against `uSimHours`. Over 336 simulated hours grime climbs the base of the walls in streaks, colours dull a little, and a green tide line settles at the waterline. Cloth, foliage, windows, and lanterns are exempt.
- `src/atmosphere.ts` keyframes sun colour temperature, sun intensity, sky, fog, ambient, exposure, and weather against the simulation clock, and derives a rainbow strength from light rain under a sun below 42 degrees. `src/sky.ts` draws the gradient dome with sun, moon, stars, meteors, and sheet-lightning flashes; the rainbow is a second additive dome drawn after the cloud sprites, so only buildings hide it. `main.ts` fires the lightning during heavy showers and follows each flash with a thunder cue. `src/postfx.ts` runs N8AO, bloom, exposure, ACES tone mapping, a black lift with saturation, a soft vignette, and optional depth of field through `postprocessing`.
- Free-standing houses pick a roof style from a seeded hash: two thirds keep the steep cap, one third takes a low hip roof. A few houses of two storeys or more keep a stone ground floor. Wall and roof colours stay palette-driven, so the variance is silhouette, not colour.
- Every human figure is built from `buildFigureGeometry` in `src/citizens.ts`: boat crews, dock workers, and barge people share the citizens' proportions, skin, hair, and hat, merged into one vertex-coloured mesh per figure.
