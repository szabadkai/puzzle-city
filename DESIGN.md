...
Water topology is derived from exposed cell edges and shared by architecture, discoveries, and harbor ambience. It identifies deterministic docks, narrow canals, and sheltered water. Rowboats, fishing boats, merchant boats, and ferries follow separate deterministic, collision-safe water lanes around the changing town envelope; the working fleet changes as docks, occupations, businesses, and discoveries develop. Wildlife and seasonal particles are grouped into a handful of draw calls and become visible only after their associated discoveries. Relationship work uses spatial buckets and a fixed comparison ceiling rather than an unbounded all-pairs scan. Three-storey homes can support a second household member; age roles, favorite-shop visit histories, shop popularity, employees, and visitors are additive fields on the existing citizen and business saves.
...
The event layer evaluates declarative condition trees against a cloned, frozen world snapshot and commits effects through narrow city, business, citizen, ambience, and presentation APIs. Conditions query topology, adjacency, height, available storefronts, water configuration, population, citizen occupations, ages and traits, time, businesses, regular-customer habits, popularity, relationships, and prior discoveries. Forty-three one-shot events are idempotent by stable ID. Five additional recurring town moments use per-event simulated-hour cooldowns reconstructed from the saved journal, so a reload cannot cause them to fire repeatedly. Each committed event records a witnessed note in the illustrated harbor journal.
...
4. **GROW system — implemented.** A reusable read-only snapshot and condition/effect graph, 43 chained one-shot discoveries, five cooldown-backed recurring moments, stable IDs, additive save migration, diegetic presentation beats, and an illustrated observation journal.
